...
- The restart waits about a minute past the reset before it fires, and reads the
  window again first. For a short while after a rollover `/usage` still reports
  the window that just ended, percentage included, and restarting on that reading
  got the next tool call denied a second later.
